...
/* Disable sched scan. Unfortunately there is no config option */
#if 0
...
#ifdef CONFIG_WAPI
	if (params->key_mgmt_suite == KEY_MGMT_WAPI_CERT ||
		params->key_mgmt_suite == KEY_MGMT_WAPI_PSK) {
...
static int wpa_driver_nl80211_deinit_p2p_cli(void *priv)
{
	...
	wpa_printf(MSG_DEBUG, "nl80211: %s\n", __func__);
	...
	if (os_strcasecmp(cmd, "STOP") == 0) {
		...
		if (os_strlen(cmd) == 13) {
			...
			wpa_printf(MSG_INFO, "Scan Channels command = %d",
				   nr);
			if(nr == 14)
				ret = wpa_driver_nl80211_set_country(priv, "JP");
			else if(nr == 13)
				ret = wpa_driver_nl80211_set_country(priv, "EU");
			else if(nr == 11)
				ret = wpa_driver_nl80211_set_country(priv, "US");
			else {
				ret = -1;
			}
			drv->scan_channels = nr;
		}
	} else {
