					 ...
					 struct wpa_scan_res *bss)
{
	...
	if ((ssid->proto & WPA_PROTO_WAPI) &&
		(wpa_scan_get_ie(bss, WLAN_EID_WAPI) ||
		wpa_scan_get_vendor_ie(bss, WAPI_IE_VENDOR_TYPE))) {
		wpa_dbg(wpa_s, MSG_DEBUG, "   selected based on WAPI IE");
		return 1;
	}
#endif /* CONFIG_WAPI */
...
#ifdef CONFIG_WAPI
	ie = wpa_scan_get_vendor_ie(bss, WAPI_IE_VENDOR_TYPE);
	wapi_ie_len = ie ? ie[1] : 0;

	if (wapi_ie_len == 0) {
		ie = wpa_scan_get_ie(bss, WLAN_EID_WAPI);
		wapi_ie_len = ie ? ie[1] : 0;
	}
#endif /* CONFIG_WAPI */
					  ...
					  union wpa_event_data *data)
{
	...
	if (!found && data->assoc_info.req_ies) {
		...
		wpa_dbg(wpa_s, MSG_DEBUG, "WAPI: (wapi) no wapi info element found "
				       ...
				       union wpa_event_data *data)
{
	...
	if (wpa_s->wapi_l2)
		/* empty implementation - for l2_packet_linux.c*/
		l2_packet_notify_auth_start(wpa_s->wapi_l2);
#endif /* CONFIG_WAPI */
					 ...
					 union wpa_event_data *data)
{
	...
	if (wpa_s->pairwise_cipher == WPA_CIPHER_SMS4) {
		wpa_msg(wpa_s, MSG_INFO, "WAPI: Ignoring MIC failure");
		return;
	}
